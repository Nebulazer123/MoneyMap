    ...
    <div className="relative pt-2">
      ...
      <div className="flex items-center gap-2">
        <div ref={containerRef} className="relative flex w-full snap-x snap-mandatory items-center gap-3 overflow-x-auto px-2 pb-2 sm:justify-center">
          {/* Animated underline */}
          <div
            className="absolute bottom-0 h-1 bg-gradient-to-r from-purple-500 to-purple-400 rounded-full transition-all duration-300 ease-out"
            style={{
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`snap-start rounded-full border px-5 py-3 text-sm font-semibold transition duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40 ${
                activeTab === tab.id
                  ? "border-purple-500/40 bg-purple-500/10 text-white shadow-[0_0_16px_rgba(168,85,247,0.2)]"
                  : "border-zinc-700 bg-zinc-900 text-zinc-300 hover:border-zinc-500 hover:bg-zinc-800 hover:text-zinc-100"
              }`}
            >
              {tab.label}
            </button>
