...
type DuplicateDecision = "confirmed" | "dismissed";

type Props = {
  duplicateClusters: DuplicateClusterView[];
  duplicateDecisions: Record<string, DuplicateDecision | undefined>;
...
export function DuplicateOverlay({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 px-3 py-6"
    >
      <GlassPanel
        variant="hero"
        tabIndex={-1}
        ...
        <div className="max-h-[70vh] overflow-y-auto p-4 sm:p-5 space-y-3 text-sm text-zinc-200">
          {duplicateClusters.length === 0 ? (
            <p className="text-zinc-400">
              No suspected duplicates for this statement.
            </p>
          ) : (
            duplicateClusters.map((cluster) => {
              ...
              return (
                <GlassPanel
                  key={cluster.key}
                  variant="card"
                  className="p-0 backdrop-blur-xl"
                >
                  <button
                    type="button"
                    ...
                    <div className="divide-y divide-zinc-800 text-xs text-zinc-200">
                      {cluster.suspiciousTransactions.map(({ tx, reason }) => {
                        ...
                        return (
                          <div
                            key={tx.id}
                            className="grid grid-cols-6 items-center px-4 py-2"
                          >
