...
export function useOwnershipAccounts({
  ...
  // Memoize candidate keys to prevent infinite effect loop
  const candidateKeys = useMemo(
    () => detectedAccountCandidates.map((c) => c.key).join(","),
  ...
  // Memoize suggested transaction IDs to prevent infinite effect loop
  const suggestedTxIds = useMemo(
    () => suggestedAccountTransactions.map((tx) => tx.id).join(","),
    [suggestedAccountTransactions]
  );

  useEffect(() => {
    if (!addBaseTransactionId) {
      setSelectedAccountTxIds((prev) => (prev.size === 0 ? prev : new Set()));
      return;
    }
    const ids = suggestedTxIds ? suggestedTxIds.split(",") : [addBaseTransactionId];
    setSelectedAccountTxIds((prev) => {
      const next = new Set(ids);
      // Avoid update if sets are equal
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [addBaseTransactionId, suggestedTxIds]);
