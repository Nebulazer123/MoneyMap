...
function getMarketStatus() {
    ...
    if (isOpen) {
        ...
        const hoursUntil = Math.floor(minutesLeft / 60);
        const minsUntil = minutesLeft % 60;
        statusMessage = `${hoursUntil}h ${minsUntil}m until close`;
    } else {
        // Time until open
        let minutesToOpen = 0;
        ...
        if (dayName === 'Sat') {
            ...
            // Sunday: 1 day until Monday 9:30
            minutesToOpen = (24 * 60 - currentMinutes) + marketOpen;
        } else if (currentMinutes < marketOpen) {
            // Before market open today (weekday)
            minutesToOpen = marketOpen - currentMinutes;
        } else if (dayName === 'Fri') {
            // Friday after close: until Monday 9:30
            minutesToOpen = (24 * 60 - currentMinutes) + (2 * 24 * 60) + marketOpen;
        } else {
        ...
        if (hoursUntil >= 24) {
            ...
            statusMessage = `${days}d ${remainingHours}h until open`;
        } else {
...
const INITIAL_HOLDINGS: Omit<StockHolding, 'currentPrice'>[] = [
    ...
    { id: '2', symbol: 'MSFT', name: 'Microsoft Corp.', shares: 10, avgCost: 350.00, addedAt: new Date('2024-03-20') },
    { id: '3', symbol: 'NVDA', name: 'NVIDIA Corp.', shares: 5, avgCost: 420.00, addedAt: new Date('2024-08-10') },
    { id: '4', symbol: 'SPY', name: 'SPDR S&P 500 ETF', shares: 15, avgCost: 440.00, addedAt: new Date('2024-01-05') },
];

// Initial watchlist
const INITIAL_WATCHLIST: WatchlistItem[] = [
    { symbol: 'GOOGL', name: 'Alphabet Inc.', addedAt: new Date() },
    { symbol: 'AMZN', name: 'Amazon.com Inc.', addedAt: new Date() },
    { symbol: 'TSLA', name: 'Tesla Inc.', addedAt: new Date() },
];
...
export function Stocks() {
    ...
    const [customStock, setCustomStock] = useState({ symbol: '', name: '', price: 0 });
    ...
    const searchStocks = useCallback(async (query: string) => {
        ...
        try {
            ...
            const localResults = Object.keys(quotes)
                ...
                .map(s => ({ symbol: s, name: quotes[s]?.name || s }));
    ...
    // Fetch detailed stock info (chart, news, insights)
    ...
    // Fetch trending stocks for browse modal
    const fetchTrendingStocks = useCallback(async () => {
        try {
            ...
            const fallback = Object.entries(quotes).slice(0, 20).map(([symbol, q]) => ({
                symbol,
                name: q.name,
            }));
    ...
    // Toggle expanded stock view
    const toggleExpandedStock = useCallback((symbol: string) => {
    ...
    }, [expandedSymbol, fetchStockDetail]);
    ...
    if (isInitialLoading) {
        return (
            <GlassCard intensity="medium" tint="lime" className="p-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex flex-col items-center justify-center py-20">
                    <Loader2 className="h-8 w-8 text-lime-400 animate-spin mb-4" />
                    <p className="text-zinc-400">Loading stock data from Yahoo Finance...</p>
                </div>
            </GlassCard>
        );
    }
        ...
        <GlassCard intensity="medium" tint="lime" className="p-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex items-center justify-between mb-8">
                <div className="text-center flex-1">
                    <h2 className="text-2xl font-semibold text-white">Stock Portfolio</h2>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className={cn(
                            "flex items-center gap-2 text-xs bg-lime-500/20 text-lime-300 px-3 py-1.5 rounded-lg hover:bg-lime-500/30 transition-colors border border-lime-500/30",
            ...
            <GlassCard className="p-5 mb-6">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        <h3 className="text-base font-semibold text-white">Search Stocks</h3>
                        <InfoTooltip text="Search any stock by ticker or company name. Powered by Yahoo Finance." />
                ...
                <div className="relative">
                    ...
                    />
                    {isSearching && (
                        <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-lime-400 animate-spin" />
                    )}
                </div>
            </GlassCard>
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-lg bg-lime-500/20 flex items-center justify-center">
                                                <span className="text-lime-400 text-xs font-bold">{result.symbol.slice(0, 2)}</span>
                                            </div>
                                            <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button
                                                onClick={async () => {
                                                    ...
                                                    setSearchQuery('');
                                                    // Fetch quote for this symbol if we don't have it
                                                    if (!quotes[result.symbol]) {
                                                        await fetchQuotes([result.symbol]);
                                                    }
                                                }}
                                                disabled={inHoldings}
                                                className={cn(
            ...
            <GlassCard className="p-5 mb-6">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <InfoTooltip text="Click any stock to view detailed chart, stats, and news" />
                    </div>
                                        ...
                                        <div className="relative p-5">
                                            ...
                                            <div className="flex items-center justify-between mb-4">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-2">
                                                            <h4 className="text-base font-semibold text-white tracking-tight">{holding.symbol}</h4>
                                                            {isExpanded ? (
                                        ...
                                        <div className="bg-zinc-900/50 border border-t-0 border-lime-500/30 rounded-b-xl p-4 animate-in slide-in-from-top-2 duration-200">
                                            {isLoadingDetail ? (
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-2">
                                                            <LineChart className="h-4 w-4 text-lime-400" />
                                                    ...
                                                    <div className="h-32 bg-zinc-900/30 rounded-xl border border-white/5 p-3">
                                                        {stockDetail.charts[chartTimeframe]?.length > 0 ? (
                                                            <MiniChart data={stockDetail.charts[chartTimeframe]} isPositive={(stockDetail.quote?.change || 0) >= 0} />
                                                        ) : (
                                                    ...
                                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            ...
                                                            <p className="text-sm font-medium text-white">
                                                                {stockDetail.quote.marketCap ? compactCurrency.format(stockDetail.quote.marketCap) : 'N/A'}
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">P/E Ratio</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {stockDetail.quote.pe?.toFixed(2) || 'N/A'}
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">52W High</p>
                                                            <p className="text-sm font-medium text-emerald-400">
                                                                {currency.format(stockDetail.quote.fiftyTwoWeekHigh || 0)}
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">52W Low</p>
                                                            <p className="text-sm font-medium text-rose-400">
                                                                {currency.format(stockDetail.quote.fiftyTwoWeekLow || 0)}
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">Open</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {currency.format(stockDetail.quote.open || 0)}
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">Day Range</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {currency.format(stockDetail.quote.low || 0)} - {currency.format(stockDetail.quote.high || 0)}
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">Volume</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {(stockDetail.quote.volume / 1e6).toFixed(2)}M
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">Avg Volume</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {stockDetail.quote.avgVolume ? (stockDetail.quote.avgVolume / 1e6).toFixed(2) + 'M' : 'N/A'}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {stockDetail.insights && (
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            ...
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                {stockDetail.insights.targetPrice && (
                                                                    <div className="text-sm">
                                                                        <span className="text-zinc-500">Target: </span>
                                                                        <span className="text-white font-medium">{currency.format(stockDetail.insights.targetPrice)}</span>
                                                                    </div>
                                                                )}
                                                            </div>
                                                        </div>
                                                    ...
                                                    {stockDetail.news.length > 0 && (
                                                        <div>
                                                            ...
                                                            <div className="space-y-2 max-h-48 overflow-y-auto">
                                                                {stockDetail.news.slice(0, 5).map((news, idx) => (
                                                                    <a
                                                                        key={idx}
                                                                        href={news.link}
                                                                        target="_blank"
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div className="flex-1 min-w-0">
                                                                            <p className="text-xs text-white line-clamp-2 group-hover:text-lime-300 transition-colors">
                                                                                {news.title}
                                                                            </p>
                                                                        ...
                                                                        <ExternalLink className="h-3 w-3 text-zinc-600 group-hover:text-lime-400 flex-shrink-0" />
                                                                    </a>
                                                                ))}
                                                            </div>
                                                        </div>
                            ...
                            return (
                                <div key={item.symbol} className="space-y-0">
                                    <div
                                        ...
                                        onClick={() => toggleExpandedStock(item.symbol)}
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={cn(
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                                        ...
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">Volume</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {((stockDetail.quote.volume || 0) / 1e6).toFixed(2)}M
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">Prev Close</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {currency.format(stockDetail.quote.previousClose || 0)}
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">EPS</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {stockDetail.quote.eps?.toFixed(2) || 'N/A'}
                                                            </p>
                                                        </div>
                                                        <div className="bg-zinc-900/30 rounded-lg p-3 border border-white/5">
                                                            <p className="text-xs text-zinc-500">Beta</p>
                                                            <p className="text-sm font-medium text-white">
                                                                {stockDetail.quote.beta?.toFixed(2) || 'N/A'}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {stockDetail.news.length > 0 && (
                                                        <div>
                                                            ...
                                                            <div className="space-y-2 max-h-40 overflow-y-auto">
                                                                {stockDetail.news.slice(0, 4).map((news, idx) => (
                                                                    <a
                                                                        key={idx}
                                                                        href={news.link}
                                                                        target="_blank"
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-full py-2 bg-lime-500/20 text-lime-300 rounded-lg hover:bg-lime-500/30 transition-colors border border-lime-500/30 text-sm font-medium"
                                                    >
                                                        Add to Portfolio
                                                    </button>
                                                </div>
            ...
            {/* Data Disclaimer */}
            <div className="flex items-start gap-2 p-4 rounded-xl bg-lime-500/10 border border-lime-500/20">
                <Wifi className="h-4 w-4 text-lime-400 mt-0.5 flex-shrink-0" />
                <div>
                ...
                <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 bg-black/60 backdrop-blur-sm" onClick={() => setShowBrowseModal(false)}>
                    <GlassCard className="w-full max-w-3xl m-4 max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
                        <div className="p-4 border-b border-white/10 flex items-center justify-between">
                            <div>
                                <h3 className="text-lg font-semibold text-white">Browse Stocks</h3>
                                <p className="text-sm text-zinc-400">Trending and popular stocks from Yahoo Finance</p>
                            </div>
                            <button
                        ...
                        <div className="p-4 overflow-y-auto max-h-[calc(80vh-100px)]">
                            ...
                            <div className="mb-6">
                                <h4 className="text-sm font-medium text-zinc-400 mb-3 flex items-center gap-2">
                                    <BarChart3 className="h-4 w-4 text-lime-400" />
                                    Loaded Stocks ({Object.keys(quotes).length})
                                </h4>
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (inWatchlist) {
                                                                ...
                                                                addToWatchlist(symbol, quote?.name);
                                                            }
                                                        }}
                                                        className={cn(
                                        ...
                                        {trendingStocks.filter(t => !quotes[t.symbol]).map((stock) => (
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-medium text-white">{stock.symbol}</p>
                                                        <p className="text-xs text-zinc-500 truncate max-w-[100px]">{stock.name}</p>
                                                    </div>
                                                </div>
                                                <button
                                                    onClick={async () => {
                                                        await fetchQuotes([stock.symbol]);
                                                    }}
            ...
            {/* Add Holding Modal */}
            {showAddHoldingModal && selectedSymbol && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <GlassCard className="w-full max-w-md p-6 m-4">
                        ...
                        <div className="flex items-center gap-4 p-4 bg-zinc-900/50 rounded-xl mb-4">
                            ...
                            <div>
                                <p className="text-base font-medium text-white">{selectedSymbol}</p>
                                <p className="text-sm text-zinc-400">{quotes[selectedSymbol]?.name}</p>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder={quotes[selectedSymbol]?.price.toString() || "0.00"}
                                    className="w-full px-3 py-2 bg-zinc-900/50 border border-white/10 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:border-lime-500/50"
                                />
                                ...
                                <div className="p-3 bg-zinc-900/30 rounded-lg">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="text-white font-medium">
                                            {currency.format(newHolding.shares * (newHolding.avgCost || quotes[selectedSymbol]?.price || 0))}
                                        </span>
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="flex gap-3 mt-6">
                            ...
                            <button
                                ...
                                className="flex-1 px-4 py-2 bg-lime-500/20 text-lime-300 rounded-lg hover:bg-lime-500/30 transition-colors border border-lime-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                Add to Portfolio
                            </button>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setCustomStock({ ...customStock, symbol: e.target.value.toUpperCase() })}
                            ...
                            {customStock.symbol && quotes[customStock.symbol] && (
                                <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                                    <p className="text-sm text-emerald-300">
                                        &quot;{customStock.symbol}&quot; is already loaded: {currency.format(quotes[customStock.symbol].price)}
                                    </p>
                                </div>
                            )}
                        </div>
